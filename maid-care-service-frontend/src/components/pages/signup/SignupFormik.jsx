...
import { auth, registerMaid } from '../../../api/auth.js';
...
const SignupFormik = () => {
  ...
  if (submit && !sent) {
    return (
      <VStack justifyContent="center" h="10rem">
        <Spinner size="xl" />
      ...
      <Wizard
        ...
        <WizardStep
          ...
          <DateField
            name="birthdate"
            label="Date of Birth"
            helperText="Maid can only be 18-80 years old"
          />
            ...
            {work_choices.map((c, index) => {
              return (
                <Box marginBottom="2" key={index}>
                  <CheckField name="work" label={c} value={c} />
                </Box>
              );
            })}
