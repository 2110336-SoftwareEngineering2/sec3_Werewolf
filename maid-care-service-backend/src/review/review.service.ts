...
import { JobState } from 'src/job/jobState';
...
export class ReviewTest{
    ...
    async testReview(): Promise<Job>{
        ...
        const Job = await this.jobService.createJob(createdCustomer._id, createJobDto);
        Job.maidId = maid._id;
        Job.save();
        ...
        maid.totalReviews = 0;
        maid.save();
        const maidReview = await this.reviewService.updateMaidRating(maid._id, 4);
